...
{
    //driver_->setSpeed(CanDriver::J1939_CAN_250K); //commented by me
...
int J1939Adapter::onRequest(const uint8_t* data, uint32_t len, uint32_t numOfResp)
{
    ...
    //driver_->clearData(); // commented by me
    ...
    //driver_->clearFilters(); // commented by me
    return sts;
}
...
void J1939Adapter::monitorImpl(uint32_t pgn, uint32_t numOfResp)
{
  /*
    ...
    if (silent) // Set silent mode
        //driver_->setSilent(true); // commented by me
    
    setFilterAndMaskForPGN(pgn);
    CmdUart* uart = CmdUart::instance();
    //uart->monitor(true); //coomented by me
    uint32_t num = 0;
    do {
        num += receiveFromEcu();
